...
export default function Section5Component(){
    ...
    React.useEffect(()=>{
        ...
        function mainSlide(){
            ...
            $('#section5 .slide-wrap').stop().animate({left: `${-101*cnt}%` }, 800, 'easeInOutExpo', function(){
                if(cnt > 3) cnt=0;  // 슬라이드 14개인데 0부터 세면 0,1,2...13이 된다 
                if(cnt < 0) cnt=3;  // 13번이 되면 0으로 돌아간다
                $('#section5 .slide-wrap').stop().animate({left: `${-101*cnt}%` }, );
            });
        ...
        let container = $('#section5 .container');

        // 데스크탑
        $('#section5 .slide-container').on({
            mousedown(e){
                container = $('#section5 .container')
                // console.log('#section5 .mousedown');
                winWidth =  $(window).innerWidth();
                clearInterval(setId);
                mouseDown=true; //마우스 다운 알림
                touchStart = e.clientX;
                dragStart = e.clientX - $('.slide-wrap').offset().left-container+((winWidth-container)/2);
