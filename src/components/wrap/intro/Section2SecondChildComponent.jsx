...
export default function Section2SecondChildComponent  ({레이플라이드})  {
    ...
    React.useEffect(()=>{
        const slideCount=$('.wrap2 .slide').length;
        const slideWrap=$('#section2 .wrap2');
    ...
    return (
        <div className="slide-container" draggable="false">
            <div className="slide-view" draggable="false">                    
                <ul className='slide-wrap wrap2' draggable="false">
                {
                    레이플라이드.map((item)=>{
                        return(
                            <li onClick={onClicklink} draggable="false" className='slide' key={item.제품코드}>
                                <div className="img-box" draggable="false">
