...
export default function Section2SixthChildComponent  ({RUNNING})  {
    ...
    React.useEffect(()=>{
        const slideCount=$('.wrap6 .slide').length;
        const slideWrap=$('#section2 .wrap6');
    ...
    return (
        <div draggable="false" className="slide-container">
            <div draggable="false" className="slide-view">                    
                <ul draggable="false" className='slide-wrap wrap6'>
                {
                    RUNNING.map((item)=>{
                        return(
                            <li draggable="false" onClick={onClicklink} className='slide' key={item.제품코드}>
